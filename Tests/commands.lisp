...
(defmacro with-gestures ((&rest gestures) &body body)
  `(progn
     ,@(loop for gesture in gestures
             collect `(define-gesture-name ,(car gesture) :keyboard ,(cdr gesture)))
     (unwind-protect (progn ,@body)
       ,@(loop for gesture in gestures
               collect `(delete-gesture-name ,(car gesture))))))
...
;;; This test is similar the previous one, however it examines :inherit-menu
;;; values that do not inherit keystrokes.
